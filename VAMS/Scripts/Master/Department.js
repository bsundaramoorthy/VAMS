    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.length > 0) {
                table = table + '<table class="display nowrap table-responsive" id="example" style="width:100%"><thead><th>' + SerialNoLang + '<th>' + DepartmentLang + '</th><th>' + ShortNameLang + '</th><th>' + EditLang + '</th><th>' + DeleteLang + '</th></thead><tbody>'
...
function Save() {
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response == -1) {
                Content = ContentMaking(DepartmentLang, Department.DepartmentName, NotSaveLang);
                ShowErrorMsg(Content);
            }
            else if (response == 1) {

                if ($('#hdnDepartmentID').val() == "0") {
                    Content = ContentMaking(DepartmentLang, Department.DepartmentName, SaveMsgLang);
                    ShowSuccessMsg(Content);
                }
                if ($('#hdnDepartmentID').val() != "0") {
                    Content = ContentMaking(DepartmentLang, Department.DepartmentName, UpdateMsgLang);
                    ShowSuccessMsg(Content);
                }
                ...
                Content = ContentMaking(DepartmentLang,Department.DepartmentName, AlreadyExistMsgLang);
                ShowErrorMsg(Content);
            }
        },
        failure: function (response) {
